#!/bin/sh
# Lightsail prepends its own /bin/sh setup before user_data. Keep this POSIX.
set -eu
export DEBIAN_FRONTEND=noninteractive
apt-get -o Acquire::Retries=3 update
apt-get -o Acquire::Retries=3 -o DPkg::Lock::Timeout=300 install -y ca-certificates curl tar xz-utils sqlite3 openssl unattended-upgrades
position_lens_tmp=$(mktemp -d)
trap 'rm -rf "$position_lens_tmp"' EXIT
cd "$position_lens_tmp"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 180 -o node.tar.xz https://nodejs.org/dist/v24.21.0/node-v24.21.0-linux-x64.tar.xz
...
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 180 -o caddy.tar.gz https://github.com/caddyserver/caddy/releases/download/v2.11.4/caddy_2.11.4_linux_amd64.tar.gz
...
for account in position-lens caddy; do
